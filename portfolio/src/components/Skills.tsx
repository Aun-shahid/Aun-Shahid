    ...
    <section className="grid gap-6 py-2">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {skillGroups.map((group) => (
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5" key={group.title}>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {group.items.map((item) => {
                  ...
                  return (
                    <span
                      key={item.label}
                      className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-bold text-slate-800 dark:border-white/10 dark:bg-slate-950/40 dark:text-slate-200"
                    >
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {deliveryFocus.map((item) => {
              const Icon = item.icon
              return (
                <div key={item.label} className="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 font-bold text-slate-800 dark:border-white/10 dark:bg-slate-950/40 dark:text-slate-200">
                  <Icon aria-hidden="true" className="shrink-0 text-teal-700" />
