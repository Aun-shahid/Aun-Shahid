...
export default function ProjectDetailsModal({ project, onClose }: ProjectDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 p-4 backdrop-blur-md sm:p-6" role="dialog" aria-modal="true" aria-labelledby="project-details-title">
      ...
      <div className="project-details-scroll relative mx-auto max-h-[calc(100vh-2rem)] max-w-6xl overflow-y-auto rounded-2xl border border-white/15 bg-slate-50 shadow-2xl shadow-slate-950/40 dark:bg-slate-950 sm:max-h-[calc(100vh-3rem)]">
        <div className="sticky top-0 z-40 flex items-center justify-between gap-4 border-b border-slate-200 bg-white/92 px-5 py-4 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/92 sm:px-6">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 p-5 text-white sm:p-8 lg:p-10">
        ...
        <div className="grid gap-6 p-5 sm:p-8 lg:p-10">
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag) => (
              <span key={tag} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
                {tag}
              </span>
            ))}
          </div>
